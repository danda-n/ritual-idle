...
const NONE: Talents = { ranks: {}, keystone: false };
...
export function pointsSpent(state: GameState, skill: SkillId): number {
  ...
  return BRANCH_IDS.reduce((n, b) => n + (t.ranks[b] ?? 0), 0) + (t.keystone ? 1 : 0);
}
...
/** The keystone opens after KEYSTONE_NEEDS points in any one branch. */
...
/** The skill's keystone effect, if taken. */
export function keystoneEffect(state: GameState, skill: SkillId): KeystoneEffect | null {
  return talentsOf(state, skill).keystone ? KEYSTONES[skill].effect : null;
}
...
/** Why this talent can't be taken, or null if it can. `branch` null means the keystone. */
export function canSpend(state: GameState, skill: SkillId, branch: BranchId | null): string | null {
  if (pointsFree(state, skill) < 1) return "No talent points to spend.";
  if (branch === null) {
    if (talentsOf(state, skill).keystone) return "Already taken.";
    if (!keystoneOpen(state, skill)) return `Needs ${KEYSTONE_NEEDS} points in one branch first.`;
    return null;
  }
