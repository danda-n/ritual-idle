...
// Sources so far: sanctum upgrades. Buffs, omens, followers and grimoire effects join later.
...
export function speedMultiplier(state: GameState, id: ActionId): number {
...
export function actionDurationMs(state: GameState, id: ActionId): number {
  return (ACTION_DEFS[id].seconds * 1000) / speedMultiplier(state, id);
}
